...
const CAT_COLORS = Object.freeze<Record<string, string>>({
  'dex':       'rgba(34,255,170,0.80)',
  'lending':   'rgba(99,179,237,0.8)',
  'l1':        'rgba(255,187,0,0.80)',
  'l2':        'rgba(180,130,255,0.80)',
  'nft':       'rgba(249,115,22,0.8)',
  'stablecoin':'rgba(226,232,240,0.6)',
  'defi':      'rgba(52,211,153,0.6)',
...
const ProtocolRow = memo(({ protocol, rank, tab, period }: RowProps) => {
  ...
  return (
    <div style={rowStyle} role="row" aria-label={protocol.name + ' fundamentals row'}>
      ...
      {tab === 'revenue' && (
        <>
          <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '10px', color: 'rgba(34,255,170,0.90)', textAlign: 'right' as const }}>
      ...
      {tab === 'ratios' && (
        <>
          <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '10px', color: protocol.priceToSalesRatio != null && protocol.priceToSalesRatio < 10 ? 'rgba(34,255,170,0.90)' : 'var(--zm-text-secondary)', textAlign: 'right' as const }}>
            {formatNull(protocol.priceToSalesRatio)}
          </span>
          <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '10px', color: protocol.priceToEarningsRatio != null && protocol.priceToEarningsRatio < 30 ? 'rgba(34,255,170,0.90)' : 'var(--zm-text-secondary)', textAlign: 'right' as const }}>
...
const Fundamentals = memo(() => {
  ...
  const tabBtnStyle = useCallback((active: boolean) => ({
    ...
    padding: '5px 14px',
    borderRadius: '5px',
    background: active ? 'var(--zm-positive-bg)' : 'var(--zm-surface-1)',
    border: '1px solid ' + (active ? 'var(--zm-positive-border)' : 'var(--zm-divider)'),
  ...
  if (isLoading) {
    return (
      <div style={{ padding: '24px', display: 'flex', flexDirection: 'column' as const, gap: '12px' }}>
        {[...Array(8)].map((_, i) => (
          <div key={i} style={{ height: '44px', background: 'var(--zm-surface-1)', borderRadius: '6px', border: '1px solid var(--zm-divider)' }} />
        ))}
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : isTablet ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: '10px' }}>
        {[
          { label: '30D REVENUE', value: formatCompact(summaryStats.total30dRevenue), color: 'var(--zm-positive)' },
          { label: '30D FEES',    value: formatCompact(summaryStats.total30dFees),    color: 'var(--zm-warning)'  },
          { label: 'TOTAL MCAP',  value: formatCompact(summaryStats.totalMcap),       color: 'var(--zm-accent)'   },
          { label: 'MEDIAN P/E',  value: summaryStats.medianPE > 0 ? summaryStats.medianPE.toFixed(1) + 'x' : '—', color: 'var(--zm-violet)' },
        ].map(stat => (
          <div key={stat.label} style={{ background: "var(--zm-card-bg)", border: "1px solid var(--zm-card-border)", borderRadius: "var(--zm-card-radius)",  padding: '12px 16px'  }}>
            ...
            {PERIODS.filter(p => tab === 'fees' ? p.key !== 'annual' : true).map(p => (
              <button
                ...
                style={{
                  ...
                  padding: '3px 8px',
                  borderRadius: '4px',
                  background: period === p.key ? 'var(--zm-surface-3)' : 'transparent',
                  border: '1px solid ' + (period === p.key ? 'var(--zm-divider-strong)' : 'var(--zm-divider)'),
                  color: period === p.key ? 'var(--zm-text-primary)' : 'var(--zm-text-faint)',
                  cursor: 'pointer',
                }}
              >
                {p.label}
      ...
      {/* Table */}
      <div style={{ background: "var(--zm-card-bg)", border: "1px solid var(--zm-card-border)", borderRadius: "var(--zm-card-radius)",  padding: '12px'  }}>
        {/* Column headers */}
