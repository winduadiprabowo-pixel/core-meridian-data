...
const LevelRow = memo(({ level, side, midPrice, isFlash }: LevelRowProps) => {
  ...
  const deltaColor = useMemo(() => {
    ...
    if (level.delta === 'down') return 'rgba(138,138,158,0.50)';
    return color;
  }, [level.delta, color]);
  ...
  return (
    <motion.div
      initial={false}
      animate={{ backgroundColor: isFlash ? (isBid ? 'rgba(34,255,170,0.12)' : 'rgba(251,113,133,0.12)') : 'transparent' }}
    ...
    >
      ...
      {/* USD Value */}
      <span style={{ color: 'rgba(138,138,158,0.30)', textAlign: 'right', position: 'relative' }}>
        {formatUsd(usdVal)}
      </span>
    </motion.div>
  );
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <span style={{ color: 'rgba(138,138,158,0.30)', marginLeft: 6 }}>
...
function drawDepth(ctx: CanvasRenderingContext2D, bids: OrderBookLevel[], asks: OrderBookLevel[], w: number, h: number) {
  ...
  bidGrad.addColorStop(0, 'rgba(34,255,170,0.05)');
  bidGrad.addColorStop(1, 'rgba(52,211,153,0.25)');
  ctx.fillStyle = bidGrad;
  ctx.fill();
  ctx.strokeStyle = 'rgba(34,255,170,0.70)';
  ctx.lineWidth = 1.5;
  ctx.stroke();
  ...
  // Mid line
  ctx.beginPath();
  ctx.strokeStyle = 'rgba(0,238,255,0.40)';
  ctx.lineWidth = 1;
  ctx.setLineDash([4, 4]);
  ctx.moveTo(midX, 0);
  ctx.lineTo(midX, h);
...
const OrderBookTile = memo(() => {
  ...
  const statusColor = useMemo(() => {
    if (ob.wsStatus === 'connected') return 'rgba(34,255,170,0.80)';
    if (ob.wsStatus === 'reconnecting') return 'rgba(255,187,0,0.80)';
