...
const LevelRow = memo(({ level, maxQty, side, index }: LevelRowProps) => {
  ...
  const barColor  = isBid ? 'rgba(34,255,170,0.12)' : 'rgba(251,113,133,0.12)';
  const priceColor = isBid ? 'rgba(34,255,170,0.90)' : 'rgba(255,68,136,0.90)';
  ...
  return (
    ...
    >
      ...
      <span style={{ color: 'rgba(138,138,158,0.70)', position: 'relative' }}>
        {level.qty.toFixed(4)}
      </span>
...
const ImbalanceBar = memo(({ imbalance }: { imbalance: number }) => {
  ...
  const barStyle = useMemo(() => ({
    ...
    width:        pct + '%',
    background:   isBid ? 'rgba(34,255,170,0.70)' : 'rgba(251,113,133,0.7)',
    borderRadius: '3px',
    willChange:   'width' as const,
  }), [isBid, pct]);
...
const WasmOrderBook = memo(({ symbol = 'BTCUSDT', basePrice = MOCK_SEED.btcPrice }: WasmOrderBookProps) => {
  ...
  const wasmBadgeStyle = useMemo(() => ({
    ...
    background:    wasm.status === 'ready' ? 'var(--zm-positive-bg)' : 'rgba(0,238,255,0.10)',
    border:        '1px solid ' + (wasm.status === 'ready' ? 'rgba(52,211,153,0.3)' : 'rgba(34,211,238,0.25)'),
    color:         wasm.status === 'ready' ? 'rgba(34,255,170,0.90)' : 'rgba(0,238,255,0.70)',
  }), [wasm.status]);
  ...
  const sideHeaderStyle = useMemo(() => ({
    ...
    letterSpacing:  '0.06em',
    borderBottom:   '1px solid var(--zm-surface-2)',
    marginBottom:   '4px',
  }), []);
  ...
  const midPriceStyle = useMemo(() => ({
    ...
    fontSize:   '14px',
    fontWeight: 700,
    color:      metrics.imbalance >= 0 ? 'var(--zm-positive)' : 'var(--zm-negative)',
  ...
  return (
    <GlassCard style={{ ...cardBg, padding: '16px' }}>
      ...
      <div style={headerStyle}>
        <div style={titleStyle}>
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
            <rect x="1" y="6" width="5" height="7" rx="1" fill="rgba(34,255,170,0.70)" />
          ...
          {DEPTH_LEVELS.map((d) => (
            <motion.button
              ...
              style={{
                ...
                fontSize:     '10px',
                fontFamily:   "'Space Mono', monospace",
                background:   depth === d ? 'rgba(0,238,255,0.15)' : 'rgba(255,255,255,0.04)',
                color:        depth === d ? 'rgba(0,238,255,0.90)' : 'rgba(138,138,158,0.50)',
      ...
      <div style={metricsRowStyle} role="group" aria-label="Order book metrics">
        ...
        <div style={metricBoxStyle}>
          <div style={{ ...metricValueStyle, color: metrics.imbalance >= 0 ? 'rgba(34,255,170,0.90)' : 'rgba(255,68,136,0.90)' }}>
