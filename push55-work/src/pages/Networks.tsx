...
const NETWORKS: readonly Network[] = Object.freeze([
  { id:'eth',      name:'Ethereum',      layer:'L1',        consensus:'PoS',       tvl:68_200_000_000, tps:14.2,    tps24hChg:-2.1,  tvl24hChg:1.8,  gasPrice:18.4,     gasUnit:'gwei',   validators:1_042_000, uptime:99.98, color:'rgba(116,185,255,1)',    active:true },
  { id:'btc',      name:'Bitcoin',       layer:'L1',        consensus:'PoW',       tvl:2_100_000_000,  tps:7,       tps24hChg:0.4,   tvl24hChg:0.2,  gasPrice:24,       gasUnit:'sat/vB', validators:0,         uptime:99.99, color:'var(--zm-warning)',    active:true },
  { id:'sol',      name:'Solana',        layer:'L1',        consensus:'PoH+PoS',   tvl:8_900_000_000,  tps:2840,    tps24hChg:12.3,  tvl24hChg:3.1,  gasPrice:0.000025, gasUnit:'SOL',    validators:1_890,     uptime:99.1,  color:'var(--zm-violet)',   active:true },
  { id:'arb',      name:'Arbitrum One',  layer:'L2',        consensus:'Rollup',    tvl:12_400_000_000, tps:184,     tps24hChg:8.7,   tvl24hChg:2.4,  gasPrice:0.11,     gasUnit:'gwei',   validators:0,         uptime:99.95, color:'var(--zm-positive)',    active:true },
  ...
  { id:'matic',    name:'Polygon PoS',   layer:'Sidechain', consensus:'PoS',       tvl:1_800_000_000,  tps:650,     tps24hChg:-3.2,  tvl24hChg:-2.1, gasPrice:60,       gasUnit:'gwei',   validators:105,       uptime:99.81, color:'rgba(180,130,255,0.80)', active:true },
  ...
  { id:'starknet', name:'StarkNet',      layer:'L2',        consensus:'ZK-Rollup', tvl:1_900_000_000,  tps:68,      tps24hChg:14.2,  tvl24hChg:4.8,  gasPrice:0.04,     gasUnit:'gwei',   validators:0,         uptime:99.76, color:'var(--zm-violet)',   active:true },
  ...
  { id:'apt',      name:'Aptos',         layer:'L1',        consensus:'PoS',       tvl:980_000_000,    tps:160_000, tps24hChg:18.9,  tvl24hChg:3.4,  gasPrice:0.001,    gasUnit:'Octa',   validators:102,       uptime:99.55, color:'rgba(34,255,170,0.80)',  active:true },
]);
...
const NetworkRow = memo(({ n, maxTvl, maxTps }: { n: Network; maxTvl: number; maxTps: number }) => {
  ...
  const tvlColor = n.tvl24hChg >= 0 ? 'var(--zm-positive)' : 'var(--zm-negative)';
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <MiniBar pct={Math.min((n.tps / maxTps) * 100, 100)} color="var(--zm-violet)" />
      </div>
      ...
      <div style={{ textAlign: 'right' }}>
        <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '11px', color: n.uptime >= 99.9 ? 'var(--zm-positive)' : n.uptime >= 99 ? 'var(--zm-warning)' : 'var(--zm-negative)' }}>
          {n.uptime.toFixed(2) + '%'}
        </div>
...
const FlowRow = memo(({ f, maxVol }: { f: Flow; maxVol: number }) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: 'center', fontSize: '16px', color: isIn ? 'rgba(34,255,170,0.80)' : 'rgba(255,68,136,0.80)' }}>
        {isIn ? '→' : '←'}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      <div>
        <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '12px', fontWeight: 600, color: isIn ? 'var(--zm-positive)' : 'var(--zm-negative)', textAlign: 'right', marginBottom: '4px' }}>{fmtVol(f.vol24h)}</div>
        <div style={{ height: '3px', background: 'var(--zm-glass-bg)', borderRadius: '2px', overflow: 'hidden' }}>
          <div style={{ height: '100%', width: pct + '%', background: isIn ? 'rgba(34,255,170,0.70)' : 'rgba(251,113,133,0.7)', borderRadius: '2px', willChange: 'transform' }} />
        </div>
      </div>
    </div>
  );
...
const Networks = memo(() => {
  ...
  const SUMMARY = useMemo(() => Object.freeze([
    { label: 'Total TVL',         value: fmtTVL(totalTvl),               color: 'var(--zm-cyan)'  },
    { label: 'Networks',          value: NETWORKS.length.toString(),       color: 'var(--zm-positive)'  },
    { label: 'Active',            value: activeCount.toString(),           color: 'var(--zm-warning)'  },
    { label: 'Cross-chain Flows', value: FLOWS.length.toString(),          color: 'var(--zm-violet)' },
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h1 style={{
            ...
            background: 'linear-gradient(135deg, var(--zm-cyan), var(--zm-positive))',
        ...
        {TABS.map((t, i) => (
          <button
            ...
            style={{
              ...
              cursor: 'pointer',
              willChange: 'transform',
              background: tab === i ? 'rgba(34,211,238,0.12)' : 'transparent',
              color:      tab === i ? 'var(--zm-cyan)' : 'var(--zm-text-faint)',
              border:     tab === i ? '1px solid rgba(34,211,238,0.3)' : '1px solid transparent',
            }}
          >
            {t}
          </button>
              ...
              <div key={n.id} style={{
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '10px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '13px', fontWeight: 600, color: 'var(--zm-violet)' }}>{fmtTPS(n.tps)}</div>
                  </div>
                  <div>
                    <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '9px', color: 'var(--zm-text-faint)', marginBottom: '2px' }}>24h TVL</div>
                    <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '12px', color: n.tvl24hChg >= 0 ? 'var(--zm-positive)' : 'var(--zm-negative)' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '12px', color: n.uptime >= 99.9 ? 'var(--zm-positive)' : 'var(--zm-warning)' }}>
                      {n.uptime.toFixed(2) + '%'}
                    </div>
                  </div>
                </div>
        ...
        <div style={gasGridStyle}>
          {[...NETWORKS].sort((a, b) => a.gasPrice - b.gasPrice).map(n => (
            <div key={n.id} style={{
              ...
              <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '18px', fontWeight: 700, color: 'var(--zm-warning)' }}>
                {n.gasPrice + ' ' + n.gasUnit}
              </div>
