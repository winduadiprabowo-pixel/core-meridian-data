...
const BookRow = memo(({
  ...
  return (
    ...
    >
      ...
      {side === 'bid' ? (
        <>
          <span style={{ position: 'relative', fontFamily: "'JetBrains Mono', monospace", fontSize: 11, width: 80, textAlign: 'right', color: 'var(--zm-text-secondary)' }}>
            {formatCompactNum(level.cumSize)}
          </span>
          <span style={{ position: 'relative', fontFamily: "'JetBrains Mono', monospace", fontSize: 11, width: 80, textAlign: 'right', color: 'var(--zm-text-secondary)' }}>
            {level.size.toFixed(4)}
          </span>
          <span style={{ position: 'relative', fontFamily: "'JetBrains Mono', monospace", fontSize: 12, fontWeight: 600, flex: 1, textAlign: 'right', color: 'var(--zm-positive)' }}>
        ...
        <>
          <span style={{ position: 'relative', fontFamily: "'JetBrains Mono', monospace", fontSize: 12, fontWeight: 600, flex: 1, textAlign: 'left', color: 'var(--zm-negative)' }}>
            {formatPrice(level.price)}
            {isGiant && <span style={{ marginLeft: 4, fontSize: 9, color: 'rgba(251,191,36,0.9)' }}>🐋</span>}
            {!isGiant && isLarge && <span style={{ marginLeft: 4, fontSize: 9, color: 'rgba(251,191,36,0.6)' }}>●</span>}
          </span>
          <span style={{ position: 'relative', fontFamily: "'JetBrains Mono', monospace", fontSize: 11, width: 80, textAlign: 'left', color: 'var(--zm-text-secondary)' }}>
            {level.size.toFixed(4)}
          </span>
          <span style={{ position: 'relative', fontFamily: "'JetBrains Mono', monospace", fontSize: 11, width: 80, textAlign: 'left', color: 'var(--zm-text-secondary)' }}>
            {formatCompactNum(level.cumSize)}
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 14, fontWeight: 700, color: 'var(--zm-warning)' }}>
          {formatPrice(midPrice)}
        </span>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: 'var(--zm-text-secondary)' }}>
      ...
      >
        <div
          style={{
            ...
            width: bidPct + '%', background: 'rgba(34,255,170,0.70)', willChange: 'width',
          }}
        />
        <div
          style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
        <span style={{ fontFamily: 'var(--font-mono-ui)', fontSize: 9, color: 'rgba(34,255,170,0.70)' }}>
...
const LiqRow = memo(({ event }: { event: LiquidationEvent }) => {
  ...
  return (
    ...
    >
      ...
      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, width: 48, fontWeight: 700, color: 'var(--zm-text-primary)' }}>
        {sym}
      </span>
      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: 'var(--zm-text-secondary)' }}>
        {formatPrice(event.lastFilledPrice)}
      </span>
      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, fontWeight: 600, marginLeft: 'auto', color: event.isMajor ? 'var(--zm-warning)' : 'var(--zm-text-secondary)' }}>
        ${formatCompactNum(event.usdValue)}
      </span>
      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, width: 24, textAlign: 'right', color: 'var(--zm-text-faint)' }}>
        {ago}
      </span>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }} role="main" aria-label="Live Order Book">
      ...
      <div style={mainGridStyle}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Activity size={14} style={{ color: 'rgba(0,238,255,0.80)' }} />
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', paddingRight: 8, borderRight: '1px solid var(--zm-accent-dim)' }}>
              ...
              <span style={{ fontFamily: 'var(--font-mono-ui)', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', flex: 1, textAlign: 'right', color: 'rgba(34,255,170,0.80)' }}>BID</span>
            </div>
            {/* Ask header */}
            <div style={{ display: 'flex', paddingLeft: 8 }}>
              <span style={{ fontFamily: 'var(--font-mono-ui)', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', flex: 1, textAlign: 'left', color: 'rgba(255,68,136,0.80)' }}>ASK</span>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Zap size={14} style={{ color: 'rgba(255,187,0,0.80)' }} />
              <span style={{ fontFamily: 'var(--font-mono-ui)', fontSize: 12, fontWeight: 600, color: 'var(--zm-text-primary)' }}>
                Liquidations
              </span>
          ...
          <div style={{
            ...
            <div style={{ padding: '8px 12px', borderRight: '1px solid var(--zm-glass-border)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <TrendingDown size={11} style={{ color: 'rgba(255,68,136,0.80)' }} />
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, fontWeight: 700, color: 'var(--zm-negative)' }}>
                  ${liqLongTotal}
                </span>
              </div>
            </div>
            <div style={{ padding: '8px 12px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <TrendingUp size={11} style={{ color: 'rgba(34,255,170,0.80)' }} />
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, fontWeight: 700, color: 'var(--zm-positive)' }}>
                  ${liqShortTotal}
                </span>
              </div>
            </div>
          ...
          <div style={{
            ...
            {liq.stats.largestEvent && (
              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, marginLeft: 'auto', color: 'rgba(255,187,0,0.80)' }}>
      ...
      <div style={statsGridStyle}>
        {[
          { label: 'Best Bid',  value: book.bids[0] ? formatPrice(book.bids[0].price) : '—', color: 'var(--zm-positive)' },
          { label: 'Best Ask',  value: book.asks[0] ? formatPrice(book.asks[0].price) : '—', color: 'var(--zm-negative)' },
          { label: 'Mid Price', value: formatPrice(book.midPrice), color: 'var(--zm-warning)' },
          { label: 'Spread',    value: formatPrice(book.spread) + ' (' + book.spreadPct.toFixed(4) + '%)', color: 'var(--zm-violet)' },
        ].map(stat => (
          <div
            key={stat.label}
            style={{
          ...
          >
            ...
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 14, fontWeight: 700, color: stat.color }}>
              {stat.value}
            </div>
          </div>
        ))}
