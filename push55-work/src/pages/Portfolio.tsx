...
const HOLDINGS: readonly Holding[] = Object.freeze([
  { symbol: 'BTC',  name: 'Bitcoin',      qty: 0.42,  avgCost: 58200, price: 97350,  color: 'var(--zm-warning)'   },
  { symbol: 'ETH',  name: 'Ethereum',     qty: 3.8,   avgCost: 2850,  price: 3420,   color: 'rgba(116,185,255,1)'   },
  { symbol: 'SOL',  name: 'Solana',       qty: 28.5,  avgCost: 145,   price: 198,    color: 'var(--zm-violet)'  },
  { symbol: 'BNB',  name: 'BNB',          qty: 5.2,   avgCost: 290,   price: 412,    color: 'rgba(251,146,60,1)'   },
  { symbol: 'ARB',  name: 'Arbitrum',     qty: 850,   avgCost: 1.12,  price: 1.48,   color: 'var(--zm-positive)'   },
  ...
  { symbol: 'USDC', name: 'USD Coin',     qty: 2400,  avgCost: 1,     price: 1,      color: 'rgba(138,138,158,0.50)' },
]);
...
const EquityCurve = memo<{ data: readonly PerfPoint[]; height: number }>(({ data, height }) => {
  ...
  useEffect(() => {
    ...
    ctx.moveTo(sx(0), sy(data[0].value));
    data.forEach((p, i) => { if (i > 0) ctx.lineTo(sx(i), sy(p.value)); });
    ctx.strokeStyle = 'rgba(0,238,255,0.90)';
    ctx.lineWidth = 1.5;
    ctx.lineJoin = 'round';
    ctx.stroke();
  }, [data, height]);
...
const HoldingsRow = memo(({ h, total }: { h: Holding; total: number }) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '11px', textAlign: 'right', color: pos ? 'rgba(34,255,170,0.90)' : 'rgba(255,68,136,0.90)' }}>
        {(pos ? '+' : '') + fmtUSD(pnl)}
      </div>
      <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '11px', textAlign: 'right', color: pos ? 'rgba(34,255,170,0.90)' : 'rgba(255,68,136,0.90)' }}>
        {fmtPct(roi)}
...
const TradeRow = memo(({ t }: { t: Trade }) => {
  ...
  return (
    ...
    >
      ...
      <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '10px', padding: '2px 6px', borderRadius: '4px', textAlign: 'center', background: isBuy ? 'var(--zm-positive-bg)' : 'rgba(251,113,133,0.1)', color: isBuy ? 'rgba(34,255,170,0.90)' : 'rgba(255,68,136,0.90)', border: '1px solid ' + (isBuy ? 'rgba(52,211,153,0.25)' : 'rgba(251,113,133,0.25)') }}>
        {t.side}
      </span>
      ...
      <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '11px', textAlign: 'right', color: t.pnl != null ? (t.pnl >= 0 ? 'rgba(34,255,170,0.90)' : 'rgba(255,68,136,0.90)') : 'var(--zm-text-faint)' }}>
...
const RiskPanel = memo(() => {
  const RISK_METRICS = useMemo(() => Object.freeze([
    { label: 'Portfolio Beta',    value: '1.24',   color: 'var(--zm-warning)',  note: 'vs BTC' },
    { label: 'Sharpe Ratio',      value: '1.82',   color: 'var(--zm-positive)',  note: '90D' },
    { label: 'Max Drawdown',      value: '-18.4%', color: 'var(--zm-negative)', note: '90D' },
    { label: 'Volatility (30D)',  value: '42.1%',  color: 'rgba(251,146,60,1)', note: 'Annualized' },
    { label: 'Correlation BTC',   value: '0.87',   color: 'var(--zm-cyan)',  note: 'High' },
    { label: 'VaR (95%)',         value: '-4.8%',  color: 'var(--zm-violet)', note: '1D' },
  ]), []);
...
const Portfolio = memo(() => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--zm-bg-base)', fontFamily: "'Space Mono', monospace" }} aria-label="Portfolio tracker" role="main">
      ...
      {/* Header */}
      <div style={{ padding: isMobile ? '16px 16px 14px' : '24px 28px 20px', borderBottom: '1px solid var(--zm-glass-border)' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {(['1D', '7D', '30D', '90D'] as const).map(r => (
              <button key={r} type="button" aria-label={'Range ' + r} style={{
                ...
                border:     r === '90D' ? '1px solid rgba(0,238,255,0.35)' : '1px solid var(--zm-glass-border)',
                background: r === '90D' ? 'rgba(0,238,255,0.10)' : 'transparent',
                color:      r === '90D' ? 'rgba(0,238,255,0.90)' : 'var(--zm-text-faint)',
              }}>{r}</button>
            ))}
          </div>
        </div>
      ...
      {/* Metric cards */}
      <div style={{ display: 'flex', gap: isMobile ? '8px' : '12px', padding: isMobile ? '12px 16px' : '16px 28px', flexWrap: 'wrap' }}>
        {cardData.map(c => <MetricCard key={c.label} label={c.label} value={c.value} subValue={c.subValue} positive={c.positive} />)}
      </div>

      {/* Tab bar */}
      <div style={{ display: 'flex', gap: '4px', padding: isMobile ? '10px 16px' : '12px 28px', borderBottom: '1px solid var(--zm-glass-border)', overflowX: 'auto' }} role="tablist" aria-label="Portfolio sections">
        {TABS.map((t, i) => (
          <button key={t} type="button" role="tab" aria-selected={tab === i} onClick={() => handleTab(i)} style={{
            ...
            background: tab === i ? 'rgba(0,238,255,0.10)' : 'transparent',
            color:      tab === i ? 'rgba(34,211,238,0.95)' : 'var(--zm-text-faint)',
          }}>{t}</button>
        ))}
      </div>
          ...
          <div style={{ background: 'var(--zm-glass-bg)', border: '1px solid var(--zm-glass-border)', borderRadius: '12px', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', gap: '20px', padding: '12px 16px', borderTop: '1px solid var(--zm-glass-border)', background: 'var(--zm-glass-bg)' }}>
              <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '10px', color: 'var(--zm-text-faint)' }}>
                {'Total realized: '}
                <span style={{ color: 'rgba(34,255,170,0.90)' }}>
                  {fmtUSD(TRADES.filter(t => t.pnl !== null).reduce((s, t) => s + (t.pnl || 0), 0))}
                </span>
              </span>
              <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '10px', color: 'var(--zm-text-faint)' }}>
                {'Wins: '}
                <span style={{ color: 'rgba(34,255,170,0.90)' }}>{winTrades}</span>
                {' / '}
