  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <svg width={size} height={size / 2 + 20} viewBox={"0 0 " + size + " " + (size / 2 + 20)} role="img" aria-label={"Fear & Greed: " + score + " " + label}>
        {/* Track */}
        <path d={"M " + (cx - r) + "," + cy + " A " + r + "," + r + " 0 0 1 " + (cx + r) + "," + cy} fill="none" stroke="var(--zm-text-secondary)" strokeWidth={14} strokeLinecap="round" />
        {/* Zone colors */}
        {[
        ...
        <text x={cx}         y={cy - r - 6} textAnchor="middle" fontSize={8}  fontFamily="monospace" fill="var(--zm-text-secondary)">50</text>
        <text x={cx + r + 2} y={cy + 14} textAnchor="middle" fontSize={8}  fontFamily="monospace" fill="rgba(16,185,129,0.6)">100</text>
      </svg>
      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: 'var(--zm-text-faint)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Fear & Greed Index</div>
    </div>
  );
});
...
const NarrativeCard = memo(({ n }: { n: NarrativeEntry }) => {
  ...
  const sColor = sBias === 'BULLISH' ? 'var(--zm-positive)' : sBias === 'BEARISH' ? 'var(--zm-negative)' : 'var(--zm-warning)';
  return (
    <div style={{ padding: 12, borderRadius: 10, background: 'var(--zm-card-bg)', border: '1px solid var(--zm-card-border)', willChange: 'transform' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, fontWeight: 700, color: 'var(--zm-cyan)' }}>{n.tag}</span>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, padding: '2px 7px', borderRadius: 4, background: sColor + '12', color: sColor, fontWeight: 700 }}>{sBias}</span>
      </div>
      <div style={{ marginBottom: 6 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: 'var(--zm-text-faint)' }}>Velocity</span>
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, fontWeight: 700, color: sColor }}>{n.velocity}</span>
        </div>
        <div style={{ height: 4, borderRadius: 50, background: 'var(--zm-grey-10)', overflow: 'hidden' }}>
          <div style={{ height: '100%', width: n.velocity + '%', borderRadius: 50, background: sColor, willChange: 'width' }} />
        </div>
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: 'var(--zm-text-faint)' }}>{(n.mentions / 1000).toFixed(0)}K mentions</span>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, fontWeight: 600, color: isUp ? 'var(--zm-positive)' : 'var(--zm-negative)' }}>{isUp ? '+' : ''}{n.change24h.toFixed(1)}% 24h</span>
      </div>
    </div>
  );
});
...
const NewsRow = memo(({ entry }: { entry: NewsEntry }) => {
  const sColor = entry.sentiment === 'BULLISH' ? 'var(--zm-positive)' : entry.sentiment === 'BEARISH' ? 'var(--zm-negative)' : 'var(--zm-warning)';
  const scoreColor = entry.score >= 0.7 ? 'var(--zm-positive)' : entry.score >= 0.4 ? 'var(--zm-warning)' : 'var(--zm-negative)';
  return (
    <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--zm-grey-06)', display: 'grid', gridTemplateColumns: '1fr 80px 50px', gap: 12, alignItems: 'center' }}>
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, padding: '1px 5px', borderRadius: 3, background: 'var(--zm-grey-10)', color: 'var(--zm-text-faint)' }}>{entry.source}</span>
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: 'var(--zm-text-faint)' }}>{entry.time}</span>
        </div>
        <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: 'var(--zm-text-primary)', margin: 0, lineHeight: 1.4 }}>{entry.headline}</p>
      </div>
      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, padding: '2px 7px', borderRadius: 4, background: sColor + '12', color: sColor, fontWeight: 700, textAlign: 'center' }}>{entry.sentiment}</span>
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, fontWeight: 700, color: scoreColor }}>{(entry.score * 100).toFixed(0)}</div>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 8, color: 'var(--zm-text-faint)' }}>score</div>
      </div>
    </div>
  );
});
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: isMobile ? 16 : isTablet ? 20 : 28, minHeight: '100vh', background: 'var(--zm-bg-base)' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(251,191,36,0.12)', border: '1px solid rgba(251,191,36,0.25)' }}>
            <Activity size={18} style={{ color: 'var(--zm-warning)' }} />
          </div>
          <div>
            <h1 style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: isMobile ? 18 : 22, fontWeight: 700, margin: 0, background: 'linear-gradient(90deg,var(--zm-warning) 0%,var(--zm-negative) 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Sentiment</h1>
            <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: 'var(--zm-text-faint)', margin: 0 }}>Fear & Greed · social volume · narratives · news · options</p>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 6, background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.2)' }}>
          ...
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: 'var(--zm-warning)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Live sentiment feed</span>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: gridCols, gap: 12 }}>
        {([
          ...
          { label: 'Bullish News', value: bullishNews + '/' + NEWS.length, color: 'var(--zm-positive)', Icon: TrendingUp },
          { label: 'Avg Social Sent.', value: avgSentiment + '%', color: 'var(--zm-cyan)', Icon: MessageCircle },
          { label: 'Bullish Put/Call', value: bullishPutCall + '/' + PUT_CALL.length, color: 'var(--zm-violet)', Icon: BarChart3 },
        ] as const).map(s => {
          const Icon = s.Icon;
          return (
            <div key={s.label} style={{ padding: 14, borderRadius: 10, background: 'var(--zm-card-bg)', border: '1px solid var(--zm-card-border)', willChange: 'transform' }}>
              ...
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: isMobile ? 16 : 20, fontWeight: 700, color: s.color }}>{s.value}</div>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }} role="tablist" aria-label="Sentiment tabs">
        {TABS.map(t => (
          <button key={t} type="button" role="tab" aria-selected={activeTab === t} aria-label={'Switch to ' + t} onClick={() => handleTab(t)} style={{ padding: '6px 14px', borderRadius: 8, fontFamily: "'JetBrains Mono', monospace", fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.05em', cursor: 'pointer', transition: 'all 0.15s', background: activeTab === t ? 'rgba(251,191,36,0.12)' : 'transparent', color: activeTab === t ? 'var(--zm-warning)' : 'var(--zm-text-faint)', border: '1px solid ' + (activeTab === t ? 'rgba(251,191,36,0.3)' : 'transparent'), willChange: 'transform' }}>{t}</button>
        ))}
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={activeTab} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} style={{ borderRadius: 12, background: 'var(--zm-card-bg)', border: '1px solid var(--zm-card-border)', overflow: 'hidden', willChange: 'transform, opacity' }}>
            ...
            <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'auto 1fr', gap: 20, alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: 'var(--zm-text-faint)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Social Sentiment by Platform</div>
                  {SOCIAL_METRICS.map(m => (
                    <div key={m.label} style={{ display: 'grid', gridTemplateColumns: '80px 1fr 50px', gap: 10, alignItems: 'center' }}>
                      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, fontWeight: 700, color: m.color }}>{m.label}</span>
                      <div style={{ height: 6, borderRadius: 50, background: 'var(--zm-grey-10)', overflow: 'hidden' }}>
                      ...
                      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, fontWeight: 700, color: m.color }}>{(m.sentiment * 100).toFixed(0)}%</span>
                    </div>
                  ))}
                </div>
              </div>
              {/* Top Narrative */}
              <div style={{ padding: 14, borderRadius: 10, background: 'var(--zm-card-bg)', border: '1px solid var(--zm-card-border)' }}>
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: 'var(--zm-accent)', textTransform: 'uppercase', marginBottom: 4 }}>Top Trending Narrative</div>
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 18, fontWeight: 700, color: 'var(--zm-cyan)', marginBottom: 2 }}>{topNarrative.tag}</div>
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: 'var(--zm-text-faint)' }}>{(topNarrative.mentions / 1000).toFixed(0)}K mentions · velocity {topNarrative.velocity}/100</div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'News Feed' && (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 50px', gap: 12, padding: '8px 16px', borderBottom: '1px solid var(--zm-grey-10)', background: 'var(--zm-surface-1)' }}>
                {['Headline', 'Sentiment', 'Score'].map(h => <span key={h} style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: 'var(--zm-text-faint)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{h}</span>)}
              </div>
          ...
          {activeTab === 'Put/Call' && (
            <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: 'var(--zm-text-faint)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Options Put/Call Ratio — below 0.7 = bullish sentiment</div>
              {PUT_CALL.map(p => {
                const changed = p.ratio - p.prevRatio;
                const biasColor = p.bias === 'BULLISH' ? 'var(--zm-positive)' : p.bias === 'BEARISH' ? 'var(--zm-negative)' : 'var(--zm-warning)';
                return (
                  <div key={p.asset} style={{ padding: 14, borderRadius: 10, background: 'var(--zm-surface-1)', border: '1px solid var(--zm-card-border)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 14, fontWeight: 700, color: 'var(--zm-text-primary)' }}>{p.asset}</span>
                        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, padding: '2px 7px', borderRadius: 4, background: biasColor + '12', color: biasColor, fontWeight: 700 }}>{p.bias}</span>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 18, fontWeight: 700, color: biasColor }}>{p.ratio.toFixed(2)}</div>
                        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: changed < 0 ? 'rgba(34,255,170,0.80)' : 'rgba(255,68,136,0.80)' }}>{changed < 0 ? '' : '+'}{changed.toFixed(2)} vs prev</div>
                      </div>
                    </div>
                    <div style={{ position: 'relative', height: 8, background: 'var(--zm-grey-10)', borderRadius: 50, overflow: 'hidden' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 8, color: 'rgba(52,211,153,0.5)' }}>Bullish zone &lt;0.7</span>
                      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 8, color: 'rgba(251,113,133,0.5)' }}>Bearish zone &gt;0.7</span>
                    </div>
                  </div>
                );
              })}
