...
const SYMBOL_COLORS: Record<ChartSymbol, string> = Object.freeze({
  ...
  BNBUSDT: 'rgba(255,187,0,0.80)',
});

const COLOR = Object.freeze({
  up:   'var(--zm-positive)',
  down: 'var(--zm-negative)',
});
...
function drawChartMainThread(
  ...
  if (mode === 'candle') {
    ...
    for (let i = 0; i < candles.length; i++) {
      ...
      ctx.strokeStyle = isUp ? 'var(--zm-positive)' : 'var(--zm-negative)';
      ctx.fillStyle   = isUp ? 'rgba(52,211,153,0.15)' : 'var(--zm-negative-bg)';
      ctx.lineWidth = 1 / dpr;
  ...
  if (crosshair) {
    ctx.strokeStyle = 'rgba(0,238,255,0.40)'; ctx.lineWidth = 1 / dpr; ctx.setLineDash([4, 4]);
    ...
    if (hoverPrice >= lo && hoverPrice <= hi) {
      ...
      ctx.fillStyle = 'var(--zm-bg-base)'; ctx.font = (9 / dpr) + 'px JetBrains Mono, monospace'; ctx.textAlign = 'center';
...
const PriceChartTile = memo(() => {
  ...
  const changeColor = useMemo(() =>
    change24h >= 0 ? 'var(--zm-positive)' : 'var(--zm-negative)', [change24h]);
  ...
  const btnBase: React.CSSProperties = {
    ...
    background: 'transparent', color: 'var(--zm-text-secondary)',
    cursor: 'pointer', transition: 'all 0.15s',
  };
  const btnActive: React.CSSProperties = {
    ...btnBase, background: 'rgba(34,211,238,0.12)',
    color: 'var(--zm-text-primary)', borderColor: 'rgba(0,238,255,0.40)',
  };

  // Backend badge color
  const backendColor = useMemo(() => {
    if (backend === 'webgpu')  return 'rgba(167,139,250,0.7)';
    if (backend === 'webgl2')  return 'rgba(0,238,255,0.70)';
    if (backend === 'webgl1')  return 'rgba(251,191,36,0.7)';
    return 'rgba(138,138,158,0.40)';
  }, [backend]);
    ...
    <GlassCard style={{ height: 420, display: 'flex', flexDirection: 'column', padding: '12px' }} accentColor={symbolColor}>
      ...
      <AnimatePresence>
        {hoverCandle && (
          ...
          >
            <span style={{ color: 'rgba(138,138,158,0.50)' }}>{formatTime(hoverCandle.t, interval)}</span>
      ...
      <div ref={containerRef} style={{ flex: 1, position: 'relative', minHeight: 0 }}>
        {loading && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: 'rgba(0,238,255,0.50)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4, fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: 'rgba(138,138,158,0.30)' }}>
