...
// ── Adaptive performance: apply class before React mount ─────────────────
(function() {
  const nav = navigator;
  ...
  const isLow = ram <= 2 || cores <= 2 || net === '2g' || net === '3g' || (conn && conn.saveData);
  if (isLow) {
    document.documentElement.classList.add('zm-low-end');
  }
  // Store for React to read
  window.__zmDeviceRam = ram;
  window.__zmDeviceCores = cores;
  window.__zmDeviceNet = net;
})();
...
requestAnimationFrame(() => {
  requestAnimationFrame(() => {
    if (typeof (window as Window & { __zmSplashDone?: () => void }).__zmSplashDone === 'function') {
      (window as Window & { __zmSplashDone?: () => void }).__zmSplashDone!();
    }
  });
});
