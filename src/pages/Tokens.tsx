...
const RISK_COLORS = Object.freeze({
  LOW:     { bg: 'rgba(52,211,153,0.15)',  border: 'rgba(52,211,153,0.35)',  text: 'rgba(52,211,153,0.9)'  },
...
const MomentumGauge = memo<{ score: number; size?: number }>(({ score, size = 36 }) => {
  ...
  useEffect(() => {
    ...
    const hue   = score >= 80 ? 'rgba(52,211,153,1)' : score >= 60 ? 'rgba(251,191,36,1)' : 'rgba(248,113,113,1)';
    ...
    <div style={cardStyle}>
      ...
      <div style={{ marginBottom: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '10px', color: token.change24h >= 0 ? 'rgba(52,211,153,0.9)' : 'rgba(248,113,113,0.9)' }}>
            {(token.change24h >= 0 ? '+' : '') + token.change24h.toFixed(1) + '% 24H'}
          </span>
          <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '10px', color: token.change7d >= 0 ? 'rgba(52,211,153,0.9)' : 'rgba(248,113,113,0.9)' }}>
