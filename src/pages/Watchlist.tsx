...
const PriceCell = memo(({ price, direction }: { price: number; direction?: string }) => {
  ...
  return (
    <span
      ref={ref}
            style={{ fontFamily: 'monospace', fontSize: 12, fontWeight: 600, borderRadius: 4, padding: '0 4px', color: 'var(--zm-text-primary)', willChange: 'transform' }}
    >
      {formatPrice(price)}
    </span>
  );
  ...
  if (editing) {
    return (
      <input
        ...
        onKeyDown={handleKey}
        maxLength={80}
        style={{ width: '100%', padding: '2px 8px', borderRadius: 4, fontSize: 11, fontFamily: 'var(--font-mono-ui)', outline: 'none', boxSizing: 'border-box', background: 'var(--zm-accent-dim)', border: '1px solid rgba(96,165,250,0.3)', color: 'var(--zm-text-primary)', minWidth: 120 }}
  ...
  return (
    <button
      ...
      aria-label={note ? 'Edit note: ' + note : 'Add note'}
            style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, fontFamily: 'var(--font-mono-ui)', transition: 'color 0.15s,background 0.15s', color: note ? 'rgba(148,163,184,0.7)' : 'rgba(148,163,184,0.25)', maxWidth: 160, willChange: 'transform' }}
...
const AddCoinPanel = memo(({ assets, watchlistIds, onAdd, onClose }: AddCoinPanelProps) => {
  ...
  return (
    ...
    >
      <div
        style={{ display: 'flex', flexDirection: 'column', borderRadius: 16, overflow: 'hidden', width: 420, maxHeight: '70vh', background: 'var(--zm-bg-base)', border: '1px solid rgba(96,165,250,0.15)', boxShadow: '0 24px 80px rgba(0,0,0,0.6)' }}
      >
        {/* Header */}
        <div
        ...
        <div style={{ padding: '12px 16px', flexShrink: 0, borderBottom: '1px solid rgba(96,165,250,0.06)' }}>
          <div style={{ position: 'relative' }}>
            ...
            <input
              ...
              style={{ width: '100%', paddingLeft: 32, paddingRight: 12, paddingTop: 8, paddingBottom: 8, borderRadius: 8, fontSize: 12, fontFamily: 'var(--font-mono-ui)', outline: 'none', boxSizing: 'border-box', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(96,165,250,0.15)', color: 'var(--zm-text-primary)' }}
            ...
            return (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontFamily: 'monospace', fontSize: 10, color: 'var(--zm-text-secondary)' }}>{a.symbol.toUpperCase()}</div>
                </div>
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ fontFamily: 'monospace', fontSize: 12, color: 'var(--zm-text-primary)' }}>{formatPrice(a.price)}</div>
...
const EmptyState = memo(({ onAdd }: { onAdd: () => void }) => (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '96px 0', gap: 16 }}>
    <div
            style={{ width: 64, height: 64, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--zm-accent-dim)', border: '1px solid rgba(96,165,250,0.15)' }}
    >
...
const Watchlist = memo(() => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          >
            ...
            <span style={{ fontSize: 10, fontFamily: 'var(--font-mono-ui)', color: 'rgba(96,165,250,0.85)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {watchlistAssets.length > 0 && (
            <>
              <div
                                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 8, fontSize: 11, fontFamily: 'var(--font-mono-ui)', background: 'rgba(52,211,153,0.08)', border: '1px solid rgba(52,211,153,0.15)', color: 'rgba(52,211,153,1)' }}
              >
                <TrendingUp size={11} /> {stats.gainers}
              </div>
              <div
                                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 8, fontSize: 11, fontFamily: 'var(--font-mono-ui)', background: 'rgba(251,113,133,0.08)', border: '1px solid rgba(251,113,133,0.15)', color: 'rgba(251,113,133,1)' }}
              >
                <TrendingDown size={11} /> {stats.losers}
              </div>
              <div
                                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 8, fontSize: 11, fontFamily: 'var(--font-mono-ui)', background: 'var(--zm-divider)', border: '1px solid rgba(148,163,184,0.12)', color: 'rgba(148,163,184,0.6)' }}
              >
        ...
        <div style={{ position: 'relative', maxWidth: 280 }}>
          ...
          <input
            ...
            aria-label="Filter watchlist coins"
                        style={{ width: '100%', paddingLeft: 32, paddingRight: 12, paddingTop: 8, paddingBottom: 8, borderRadius: 8, fontSize: 12, fontFamily: 'var(--font-mono-ui)', outline: 'none', boxSizing: 'border-box', background: 'var(--zm-glass-bg)', border: '1px solid var(--zm-glass-border)', color: 'var(--zm-text-primary)' }}
            onFocus={e => { e.currentTarget.style.borderColor = 'rgba(96,165,250,0.35)'; }}
      ...
      {/* Table or empty */}
      {state.entries.length === 0 ? (
        <div style={{ background: 'var(--zm-glass-bg)', border: '1px solid var(--zm-glass-border)', borderRadius: '12px', position: 'relative' }}>
          <EmptyState onAdd={() => setShowAdd(true)} />
        </div>
      ) : (
        <div style={{ overflow: 'hidden', background: 'var(--zm-glass-bg)', border: '1px solid var(--zm-glass-border)', borderRadius: '12px', position: 'relative' }}>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }} role="grid" aria-label="Watchlist assets">
              <thead>
                <tr style={{ borderBottom: '1px solid var(--zm-glass-border)', background: 'var(--zm-surface-1)' }}>
                  {SORT_COLS.map(col => (
                    <th
                      key={col.key}
              ...
              <tbody>
                {sorted.length === 0 ? (
                  ...
                  return (
                    <tr
                      key={asset.id}
                                            style={{ transition: 'background 0.15s', borderBottom: '1px solid var(--zm-glass-border)', willChange: 'transform' }}
                      onMouseEnter={e => { e.currentTarget.style.background = 'rgba(96,165,250,0.03)'; }}
                      onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}
                    >
                      {/* Added date */}
                      <td style={{ padding: '10px 16px', fontFamily: 'monospace', fontSize: 10, color: 'var(--zm-text-faint)', width: 60 }}>
                        {addedDate}
                      </td>
                      ...
                      <td style={{ padding: '10px 12px', minWidth: 180 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <div>
                            ...
                            <div style={{ fontFamily: 'monospace', fontSize: 10, color: 'var(--zm-text-faint)' }}>{asset.symbol.toUpperCase()}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      {/* MCAP */}
                      <td style={{ padding: '10px 12px', textAlign: 'right', fontFamily: 'monospace', fontSize: 12, color: 'var(--zm-text-secondary)', width: 110 }}>
          ...
          {/* Footer */}
          <div
                        style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 16px', borderTop: '1px solid var(--zm-glass-border)', background: 'rgba(255,255,255,0.01)' }}
          >
