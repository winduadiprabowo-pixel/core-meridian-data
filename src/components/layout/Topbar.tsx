/**
 * Topbar.tsx — ZERØ MERIDIAN 2026 push87
 * TOTAL ROMBAK — slim, bersih, props baru topOffset/height/sidebarWidth
 */
...
const Topbar: React.FC<TopbarProps> = ({ onMenuToggle, sidebarExpanded, topOffset, height, sidebarWidth }) => {
  ...
  const toggleTheme = useCallback(() => { if (mountedRef.current) setTheme(isDark ? 'light' : 'dark'); }, [isDark, setTheme]);
  ...
  return (
    <header role="banner" style={topbarStyle}>
      ...
      {/* Brand — desktop only */}
      {!compact && (
        <span style={{ fontFamily:"'Space Mono',monospace",fontSize:'12px',fontWeight:700,letterSpacing:'0.13em',color:'rgba(210,215,240,0.85)',flexShrink:0 }}>
          ZERØ MERIDIAN
        </span>
      )}
      ...
      {!compact && (
        <nav style={{ display:'flex',gap:2,marginLeft:14 }}>
          {[['Dashboard','/dashboard'],['Markets','/markets'],['DeFi','/defi'],['Portal','/']].map(([l,p]) => (
            <a key={p} href={p} style={{ padding:'4px 10px',borderRadius:6,fontSize:'11px',fontFamily:"'Space Mono',monospace",color:'rgba(95,100,130,1)',textDecoration:'none',letterSpacing:'0.03em',transition:'all 0.12s' }}
      ...
      <div style={{ display:'flex',alignItems:'center',gap: isMobile ? 5 : 7,flexShrink:0 }}>
        ...
        {!isMobile && (
          <span style={{ fontFamily:"'Space Mono',monospace",fontSize:'10px',color:'rgba(70,75,100,1)',letterSpacing:'0.08em',whiteSpace:'nowrap' as const,minWidth:68,textAlign:'right' as const }}>
            {time}
          </span>
        )}
        ...
        {!isMobile && (
          <div style={{ display:'flex',alignItems:'center',gap:5,background:'rgba(61,214,140,0.06)',border:'1px solid rgba(61,214,140,0.14)',borderRadius:14,padding:'3px 9px',fontSize:'9px',fontFamily:"'Space Mono',monospace",color:'rgba(61,214,140,0.7)',letterSpacing:'0.09em',flexShrink:0 }}>
            <motion.div style={{ width:5,height:5,borderRadius:'50%',background:'rgba(61,214,140,0.85)',flexShrink:0 }}
              animate={rm ? {} : { opacity:[1,0.3,1] }} transition={{ duration:1.8,repeat:Infinity }} />
            LIVE
          </div>
        )}

        <motion.button style={{ width:30,height:30,borderRadius:'50%',background:'rgba(79,127,255,0.1)',border:'1px solid rgba(79,127,255,0.18)',cursor:'pointer',fontSize:'11px',fontWeight:700,color:'rgba(79,127,255,0.9)',fontFamily:"'Space Mono',monospace",flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center' }}
