...
const SYMBOL_COLORS: Record<string, string> = Object.freeze({
  ...
  XRPUSDT:  'var(--zm-cyan)',
  ADAUSDT:  'var(--zm-positive)',
  AVAXUSDT: 'var(--zm-negative)',
  DOGEUSDT: 'rgba(251,191,36,0.5)',
  LINKUSDT: 'rgba(0,238,255,0.70)',
  DOTUSDT:  'var(--zm-violet)',
});
...
function rateToColor(rate: number): string {
  if (rate > 0.001)  return 'var(--zm-positive)';   // very positive
  if (rate > 0.0002) return 'rgba(34,255,170,0.70)';
  if (rate > 0)      return 'rgba(52,211,153,0.4)';
  if (rate === 0)    return 'rgba(138,138,158,0.40)';
  if (rate > -0.0002)return 'rgba(251,113,133,0.4)';
  if (rate > -0.001) return 'rgba(251,113,133,0.7)';
  return 'var(--zm-negative)';                       // very negative
}
...
const FundingBarChart = memo(({ rates }: FundingBarChartProps) => {
  ...
  return (
    ...
    >
      {/* Zero line */}
      <line x1={midX} y1={0} x2={midX} y2={totalH} stroke="rgba(0,238,255,0.15)" strokeWidth={1} strokeDasharray="3,3" />

      {rates.map((r, i) => {
        const y = i * (barH + barGap);
        const pct = Math.abs(r.rate) / maxAbsRate;
        const color = rateToColor(r.rate);
        const symbolColor = SYMBOL_COLORS[r.symbol] ?? 'rgba(138,138,158,0.70)';
...
const FundingSummary = memo(({ rates }: SummaryProps) => {
  ...
  return (
    <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
      <div style={statStyle}>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 8, color: 'rgba(138,138,158,0.40)', letterSpacing: '0.1em' }}>AVG RATE</span>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: rateToColor(stats.avg) }}>
          {stats.avg >= 0 ? '+' : ''}{formatRate(stats.avg)}
        </span>
      </div>
      <div style={statStyle}>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 8, color: 'rgba(138,138,158,0.40)', letterSpacing: '0.1em' }}>LONGS PAY</span>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: 'rgba(34,255,170,0.90)' }}>
          {stats.positive}/{rates.length}
        </span>
      </div>
      <div style={statStyle}>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 8, color: 'rgba(138,138,158,0.40)', letterSpacing: '0.1em' }}>SHORTS PAY</span>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: 'rgba(255,68,136,0.90)' }}>
          {stats.negative}/{rates.length}
        </span>
      </div>
      <div style={statStyle}>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 8, color: 'rgba(138,138,158,0.40)', letterSpacing: '0.1em' }}>HIGHEST</span>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: 'rgba(34,255,170,0.90)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        <div>
          ...
          <div style={{
            ...
            color: 'rgba(138,138,158,0.35)', marginTop: 2,
          }}>
            BINANCE PERP · 8H INTERVAL
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 8, color: 'rgba(138,138,158,0.35)', marginBottom: 2 }}>
            NEXT FUNDING
          </div>
