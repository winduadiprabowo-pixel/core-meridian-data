...
const TradingViewChart = memo(({
  ...
  useEffect(() => {
    ...
    async function init(): Promise<void> {
      ...
      // push36: rAF ensures layout is complete on mobile before reading offsetWidth
      await new Promise(res => requestAnimationFrame(() => res(undefined)));
      if (!mountedRef.current || !containerRef.current) return;
      const w = containerRef.current.offsetWidth || 600;
      const h = chartHRef.current;
      ...
      chart = lw.createChart(containerRef.current, {
        ...
        layout: {
          ...
          textColor:   'rgba(138,138,158,0.50)',
      ...
      candleRef.current = chart.addCandlestickSeries({
        upColor:         'var(--zm-positive)',
        downColor:       'var(--zm-negative)',
        borderUpColor:   'var(--zm-positive)',
        borderDownColor: 'var(--zm-negative)',
        wickUpColor:     'rgba(52,211,153,0.6)',
        wickDownColor:   'rgba(251,113,133,0.6)',
      });

      volRef.current = chart.addHistogramSeries({
        color:        'rgba(0,238,255,0.15)',
        priceScaleId: 'volume',
      ...
      const ro = new ResizeObserver(() => {
        ...
        if (newW > 0) chartRef.current.resize(newW, chartHRef.current);
      });
  ...
  // ── Sync chart when height prop changes
  useEffect(() => {
    if (!chartRef.current || !containerRef.current) return;
    const newW = containerRef.current.offsetWidth;
    if (newW > 0) chartRef.current.resize(newW, chartH);
  }, [chartH]);
  ...
  const changeStyle = useMemo(() => ({
    ...
    color: priceChange == null
      ? 'rgba(138,138,158,0.50)'
      : priceChange >= 0 ? 'var(--zm-positive)' : 'var(--zm-negative)',
  }), [priceChange]);
    ...
    <GlassCard style={CARD_STYLE}>
          ...
          {SYMBOLS.map(sym => (
            <button
              ...
              style={{
                ...
                color: symbol === sym ? SYMBOL_COLORS[sym] : 'rgba(138,138,158,0.50)',
          ...
          {INTERVALS.map(iv => (
            <button
              ...
              style={{
                ...
                background: interval === iv ? 'rgba(34,211,238,0.14)' : 'rgba(255,255,255,0.025)',
                border: '1px solid ' + (interval === iv
                  ? 'rgba(34,211,238,0.3)'
                  : 'rgba(255,255,255,0.06)'),
                color: interval === iv ? 'var(--zm-cyan)' : 'rgba(138,138,158,0.40)',
                willChange: 'transform',
              }}
            >
              {iv}
      ...
      <div style={{ position: 'relative' as const }}>
        {loading && (
          ...
          >
            <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '10px', color: 'rgba(138,138,158,0.50)', letterSpacing: '0.1em' }}>
              LOADING...
            </span>
          </motion.div>
        )}
        <div
          ...
          role="img"
          aria-label={'Advanced chart ' + SYMBOL_LABELS[symbol] + ' ' + interval}
        />
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{
          ...
          border: '1px solid rgba(34,255,170,0.12)', borderRadius: '3px',
