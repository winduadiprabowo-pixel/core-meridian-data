 ...
 * Data: Blockchain Data + On-Chain Analytics + Market Data + On-Chain Intelligence.
...
import { useMarket Data, CC_SYMBOLS, type OHLCVCandle } from '@/hooks/useMarket Data';
...
const BAR_MAX_W = Object.freeze(200);
...
const StatBadge = memo(({ label, value, accent, icon }: StatBadgeProps) => (
  <div
    style={{ flex: 1, minWidth: 130, borderRadius: 12, padding: 16, display: 'flex', flexDirection: 'column', gap: 6, background: 'var(--zm-card-bg)', border: '1px solid var(--zm-card-border)', willChange: 'transform' }}
  >
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <span style={{ color: accent }}>{icon}</span>
      <span
        style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--zm-text-faint)' }}
      >
        {label}
      </span>
    </div>
    <span
      style={{ fontSize: 20, fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, color: 'var(--zm-text-primary)' }}
    >
      {value}
    </span>
  </div>
...
const OHLCVChart = memo(({ candles, symbol }: OHLCVChartProps) => {
  ...
  useEffect(() => {
    ...
    // Symbol label
    ctx.fillStyle  = 'rgba(0,238,255,0.80)';
...
const NetflowRow = memo(({ item, maxAbs }: NetflowRowProps) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingTop: 8, paddingBottom: 8, borderBottom: '1px solid rgba(148,163,184,0.08)' }}>
      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 14, width: 112, color: 'var(--zm-text-primary)' }}>
        {item.exchange}
      </span>
  ...
  return (
    <motion.div
      initial={{ opacity: 0, x: -8 }}
            animate={{ opacity: 1, x: 0,  display:'flex', alignItems:'center', gap:12, paddingTop:10, paddingBottom:10, borderBottom:'1px solid var(--zm-grey-06)', willChange:'transform, opacity' }}
    >
      <Icon size={14} style={{ flexShrink: 0 }} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: 'var(--zm-text-secondary)' }}>
            {truncateAddr(swap.sender)}
          </span>
          <span
            style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4, fontFamily: "'JetBrains Mono', monospace", textTransform: 'uppercase', background: 'rgba(148,163,184,0.10)', color }}
          >
            {swap.type}
          </span>
        </div>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: 'var(--zm-text-faint)' }}>
...
const PoolRow = memo(({ pool, rank }: PoolRowProps) => (
  <div
    style={{ display: 'grid', alignItems: 'center', gap: 12, paddingTop: 10, paddingBottom: 10, gridTemplateColumns: '24px 1fr 90px 90px 70px 70px', borderBottom: '1px solid var(--zm-divider)' }}
  >
    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, textAlign: 'center', color: 'var(--zm-text-faint)' }}>
      {rank}
    </span>
    <div>
      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 14, fontWeight: 700, color: 'var(--zm-text-primary)' }}>
        {pool.token0Symbol}/{pool.token1Symbol}
      </div>
      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: 'var(--zm-text-faint)' }}>
        {pool.feeTier.toFixed(2)}% fee
      </div>
    </div>
    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 14, textAlign: 'right', color: 'var(--zm-text-secondary)' }}>
      {formatCompact(pool.tvlUsd)}
    </span>
    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 14, textAlign: 'right', color: 'var(--zm-text-secondary)' }}>
      {formatCompact(pool.volumeUsd24h)}
    </span>
    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 14, textAlign: 'right', color: 'var(--zm-positive)' }}>
      {formatPct(pool.apr)}
    </span>
    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, textAlign: 'right', color: 'var(--zm-text-faint)' }}>
...
const TrendingCard = memo(({ token }: TrendingCardProps) => {
  ...
  return (
    <div
      style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 12, borderRadius: 8, background: 'var(--zm-card-bg)', border: '1px solid var(--zm-card-border)', willChange: 'transform' }}
    >
      {token.thumb ? (
        <img
          src={token.thumb}
        ...
        <div
          style={{ width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, flexShrink: 0, background: 'rgba(0,238,255,0.15)', color: 'var(--zm-cyan)' }}
        >
          {token.symbol.slice(0, 2)}
        </div>
      )}
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 14, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: 'var(--zm-text-primary)' }}>
          {token.symbol}
        </div>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', textTransform: 'capitalize', color: 'var(--zm-text-faint)' }}>
          {token.chain}
        </div>
      </div>
...
const TabButton = memo(({ id, label, active, onClick }: TabButtonProps) => {
  ...
  return (
    <button
      ...
      role="tab"
      style={{ position: 'relative', padding: '8px 16px', fontFamily: "'JetBrains Mono', monospace", fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.05em', borderRadius: 8, transition: 'color 0.15s,background 0.15s', cursor: 'pointer', background: active?'rgba(0,238,255,0.15)':'transparent', color: active?'var(--zm-cyan)':'var(--zm-text-faint)', border: active?'1px solid rgba(34,211,238,0.3)':'1px solid transparent', willChange: 'transform' }}
    >
      {label}
    </button>
  );
...
const LoadingRow = memo(() => (
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, paddingTop: 24, paddingBottom: 24, color: 'var(--zm-text-faint)' }}>
    ...
    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12 }}>Fetching on-chain data…</span>
  </div>
));
    ...
    {CC_SYMBOLS.map(sym => {
      ...
      return (
        <button
          ...
          aria-label={sym + ' asset selector'}
          style={{ padding: '4px 12px', borderRadius: 4, fontFamily: "'JetBrains Mono', monospace", fontSize: 12, textTransform: 'uppercase', cursor: 'pointer', background: isActive?'rgba(34,211,238,0.2)':'var(--zm-divider)', color: isActive?'var(--zm-cyan)':'var(--zm-text-faint)', border: isActive?'1px solid rgba(0,238,255,0.40)':'1px solid transparent', willChange: 'transform' }}
        >
          {sym}
        </button>
      );
...
const OnChain = memo(() => {
  ...
  const cc       = useMarket Data(ccSymbol);
  ...
  const dexStatsItems = useMemo(() => {
    ...
    return [
      { label: 'DEX Volume 24h', value: formatCompact(s.totalVolumeUsd), accent: 'var(--zm-cyan)',  icon: <BarChart3 size={14} /> },
  ...
  return (
    <div
            style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: '16px', minHeight: '100vh', background: 'var(--zm-bg-base)' }}
    >
      {/* Header */}
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
        <div>
          <h1
                        style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 24, fontWeight: 700, letterSpacing: '-0.025em', color: 'var(--zm-text-primary)' }}
          >
            On-Chain Analytics
          </h1>
          <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, marginTop: 4, color: 'var(--zm-text-faint)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button
            ...
            aria-label="Refresh all on-chain data"
                        style={{ padding: 8, borderRadius: 8, transition: 'color 0.15s,background 0.15s', cursor: 'pointer', background: 'var(--zm-card-bg)', border: '1px solid var(--zm-card-border)', color: 'var(--zm-text-faint)', willChange: 'transform' }}
          >
            <RefreshCw size={14} />
          </button>
        </div>
      ...
      {anyError && (
        <div
                    style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', borderRadius: 8, fontSize: 14, fontFamily: "'JetBrains Mono', monospace", background: 'rgba(251,113,133,0.08)', border: '1px solid rgba(251,113,133,0.2)', color: 'var(--zm-negative)' }}
        >
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'overview' && (
          ...
          >
            ...
            {/* Trending On-Chain */}
            <div
                            style={{ borderRadius: 12, padding: 16, background: 'var(--zm-card-bg)', border: '1px solid var(--zm-card-border)' }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                <Flame size={14} style={{ color: 'var(--zm-negative)' }} />
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700, color: 'var(--zm-text-primary)' }}>
                  Trending On-Chain
                </span>
              </div>
        ...
        {activeTab === 'whales' && (
          <motion.div
            ...
            style={{ willChange: 'transform, opacity' }}
          >
            {/* Uniswap V3 whale swaps from Blockchain Data */}
            <div
                            style={{ borderRadius: 12, padding: 16, background: 'var(--zm-card-bg)', border: '1px solid var(--zm-card-border)' }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                <Wallet size={14} style={{ color: 'var(--zm-cyan)' }} />
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700, color: 'var(--zm-text-primary)' }}>
                  Uniswap V3 — Whale Swaps  ($100k+)
                </span>
                <span
                                    style={{ marginLeft: 'auto', fontFamily: "'JetBrains Mono', monospace", fontSize: 10, padding: '2px 8px', borderRadius: 4, background: 'rgba(0,238,255,0.10)', color: 'rgba(0,238,255,0.80)' }}
                >
                  BLOCKCHAIN DATA
                </span>
              </div>
              {graph.loading ? <LoadingRow /> : (
                <div>
                  {graph.whaleSwaps.length === 0 ? (
                    <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, paddingTop: 16, paddingBottom: 16, textAlign: 'center', color: 'var(--zm-text-faint)' }}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                ...
                <span
                                    style={{ marginLeft: 'auto', fontFamily: "'JetBrains Mono', monospace", fontSize: 10, padding: '2px 8px', borderRadius: 4, background: 'var(--zm-violet-bg)', color: 'rgba(180,130,255,0.80)' }}
                >
                  ON-CHAIN ANALYTICS
                </span>
              </div>
                  ...
                  {dune.whaleMoves.length === 0 ? (
                    ...
                    dune.whaleMoves.slice(0, 20).map((w, idx) => {
                      ...
                      return (
                        <div
                          key={w.wallet + idx}
                                                    style={{ display: 'flex', alignItems: 'center', gap: 12, paddingTop: 8, paddingBottom: 8, borderBottom: '1px solid var(--zm-divider)' }}
                        >
                          <span
                                                        style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, padding: '2px 6px', borderRadius: 4, textTransform: 'uppercase', background: 'var(--zm-divider)', color: actColor, flexShrink: 0 }}
                          >
                            {w.action}
                          </span>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: 'var(--zm-text-secondary)' }}>
                          ...
                          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, fontSize: 14, color: actColor }}>
          ...
          >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                ...
                <span
                                    style={{ marginLeft: 'auto', fontFamily: "'JetBrains Mono', monospace", fontSize: 10, padding: '2px 8px', borderRadius: 4, background: 'var(--zm-positive-bg)', color: 'rgba(34,255,170,0.80)' }}
                >
                  BLOCKCHAIN DATA
                </span>
              </div>
              ...
              {/* Header row */}
              <div
                style={{ display: 'grid', gap: 12, paddingTop: 8, paddingBottom: 8, marginBottom: 4, gridTemplateColumns: '24px 1fr 90px 90px 70px 70px', borderBottom: '1px solid rgba(138,138,158,0.15)' }}
              >
                {['#', 'Pair', 'TVL', 'Vol 24h', 'APR', 'Txns'].map(h => (
                  <span key={h} style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', textAlign: 'right', color: 'var(--zm-text-faint)' }}>
                    {h}
                  </span>
                ))}
              </div>
          ...
          >
            ...
            >
              <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                <BarChart3 size={14} style={{ color: 'var(--zm-cyan)' }} />
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700, color: 'var(--zm-text-primary)' }}>
                  90-Day OHLCV  —  Daily Candles
                </span>
                <span
                                    style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, padding: '2px 8px', borderRadius: 4, background: 'rgba(0,238,255,0.10)', color: 'rgba(0,238,255,0.80)' }}
                >
                  MARKET DATA
                </span>
              </div>
              ...
              {!cc.loading && cc.topPairs.length > 0 && (
                <div style={{ marginTop: 16 }}>
                  <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8, color: 'var(--zm-text-faint)' }}>
                  ...
                  >
                    {cc.topPairs.slice(0, 8).map(p => (
                      ...
                      >
                        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, fontWeight: 700, color: 'var(--zm-text-primary)' }}>
                          {p.exchange}
                        </div>
                        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: 'var(--zm-text-faint)' }}>
                          Vol: {formatCompact(p.volume24hUsd)}
                        </div>
                        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 14, fontWeight: 700, marginTop: 2, color: 'var(--zm-positive)' }}>
          ...
          >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <Globe size={14} style={{ color: 'var(--zm-cyan)' }} />
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700, color: 'var(--zm-text-primary)' }}>
                  Exchange Netflow
                </span>
                <span
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16, marginTop: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <div style={{ width: 12, height: 12, borderRadius: 4, background: 'rgba(34,255,170,0.70)' }} />
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: 'var(--zm-text-faint)' }}>Outflow (bullish)</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <div style={{ width: 12, height: 12, borderRadius: 4, background: 'rgba(251,113,133,0.7)' }} />
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: 'var(--zm-text-faint)' }}>Inflow (bearish)</span>
                </div>
              </div>
