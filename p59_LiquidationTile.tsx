...
const MAX_BUBBLES = getProfile().maxBubbles; // adaptive: 15 low / 30 mid / 60 high
const BUBBLE_LIFE = getProfile().isLowEnd ? 6000 : 12000; // ms — shorter on low-end

const COLOR = Object.freeze({
  long:      'var(--zm-negative)',    // SELL = long liquidated = red
  longGlow:  'rgba(251,113,133,0.6)',
  short:     'var(--zm-positive)',     // BUY = short liquidated = green
  shortGlow: 'rgba(52,211,153,0.6)',
  whale:     'var(--zm-warning)',
  whaleGlow: 'rgba(251,191,36,0.7)',
  text:      'rgba(226,232,240,0.9)',
  faint:     'rgba(138,138,158,0.40)',
  grid:      'rgba(0,238,255,0.04)',
});
...
const LiquidationTile = memo(() => {
  ...
  const statusColor = useMemo(() => {
    if (wsStatus === 'connected') return 'rgba(34,255,170,0.80)';
    if (wsStatus === 'reconnecting') return 'rgba(255,187,0,0.80)';
    return 'rgba(255,68,136,0.80)';
  }, [wsStatus]);
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 8, color: 'rgba(138,138,158,0.30)' }}>LONGS LIQ</div>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: 'rgba(255,68,136,0.90)' }}>
              {formatCompact(stats.totalLongLiqUsd)}
            </div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 8, color: 'rgba(138,138,158,0.30)' }}>SHORTS LIQ</div>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: 'rgba(34,255,170,0.90)' }}>
              {formatCompact(stats.totalShortLiqUsd)}
            </div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 8, color: 'rgba(138,138,158,0.30)' }}>PER MIN</div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 10, fontFamily: "'JetBrains Mono', monospace", fontSize: 8 }}>
          ...
          <span style={{ color: 'rgba(34,255,170,0.70)' }}>● SHORT LIQ</span>
