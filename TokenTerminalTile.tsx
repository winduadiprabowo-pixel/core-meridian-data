...
const TAB_CONFIG = Object.freeze({
  revenue: { label: 'Revenue 30D', color: 'var(--zm-positive)' },
  fees:    { label: 'Fees 30D',    color: 'var(--zm-cyan)' },
} as const);
  ...
  <>
    {Array.from({ length: n }).map((_, i) => (
      <div key={i} style={{ ...ROW_STYLE, opacity: 1 - i * 0.08 }}>
        ...
        <div style={{ flex: 1, height: 10, borderRadius: 4, background: 'var(--zm-surface-2)' }} />
  ...
  return (
    <GlassCard style={CARD_STYLE}>
      ...
      <div style={headerStyle}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none" aria-hidden="true">
            <rect x="1" y="7" width="2" height="4" fill="rgba(34,255,170,0.70)" rx="0.5" />
            <rect x="5" y="4" width="2" height="7" fill="rgba(52,211,153,0.85)" rx="0.5" />
            <rect x="9" y="1" width="2" height="10" fill="var(--zm-positive)" rx="0.5" />
          </svg>
          ...
          {(Object.keys(TAB_CONFIG) as TabKey[]).map(tab => (
            <button
              ...
              style={{
                ...
                padding: '3px 8px',
                borderRadius: '4px',
                background: activeTab === tab ? 'rgba(34,255,170,0.12)' : 'var(--zm-surface-1)',
                border: '1px solid ' + (activeTab === tab ? 'rgba(52,211,153,0.3)' : 'rgba(255,255,255,0.06)'),
                color: activeTab === tab ? 'var(--zm-positive)' : 'rgba(138,138,158,0.50)',
                cursor: 'pointer',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '0 0 4px' }}>
        <div style={{ width: 24 }} />
        <div style={{ flex: 1, fontFamily: "'Space Mono', monospace", fontSize: '8px', color: 'rgba(138,138,158,0.35)', letterSpacing: '0.1em' }}>PROTOCOL</div>
        <div style={{ width: 70, textAlign: 'right' as const, fontFamily: "'Space Mono', monospace", fontSize: '8px', color: 'rgba(138,138,158,0.35)', letterSpacing: '0.1em' }}>{cfg.label.split(' ')[0]}</div>
        <div style={{ width: 40, textAlign: 'right' as const, fontFamily: "'Space Mono', monospace", fontSize: '8px', color: 'rgba(138,138,158,0.35)', letterSpacing: '0.1em' }}>P/E</div>
      </div>
      ...
      <div style={{ overflowY: 'auto' as const, maxHeight: '260px' }}>
        {isLoading ? (
          ...
          <div style={{ textAlign: 'center' as const, padding: '24px 0', fontFamily: "'Space Mono', monospace", fontSize: '10px', color: 'rgba(138,138,158,0.30)' }}>
          ...
          return (
            ...
            >
              {/* Rank */}
              <div style={{ width: 16, fontFamily: "'Space Mono', monospace", fontSize: '9px', color: 'rgba(138,138,158,0.30)', textAlign: 'right' as const }}>
                {i + 1}
              </div>
              ...
              <div style={{
                ...
                {p.logoUrl ? (
                  <img src={'/api/img?u=' + encodeURIComponent(p.logoUrl)} alt="" crossOrigin="anonymous" style={{ width: 16, height: 16, borderRadius: '50%' }} />
                ) : (
              ...
              {/* P/E */}
              <div style={{ width: 40, textAlign: 'right' as const, fontFamily: "'Space Mono', monospace", fontSize: '9px', color: p.priceToEarningsRatio != null ? 'rgba(255,187,0,0.80)' : 'rgba(148,163,184,0.2)' }}>
      ...
      {/* Footer */}
      <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '8px', color: 'rgba(138,138,158,0.25)', letterSpacing: '0.06em', marginTop: 'auto' }}>
