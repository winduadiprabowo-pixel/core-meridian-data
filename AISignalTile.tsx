...
const ANOMALY_CONFIG = Object.freeze({
  NONE:    { color: 'rgba(138,138,158,0.50)',  bg: 'var(--zm-grey-06)', label: '◆ NONE' },
  LOW:     { color: 'rgba(34,255,170,0.80)',   bg: 'rgba(52,211,153,0.08)',  label: '▲ LOW' },
  MEDIUM:  { color: 'var(--zm-warning)',     bg: 'rgba(251,191,36,0.08)',  label: '◈ MEDIUM' },
  HIGH:    { color: 'var(--zm-negative)',    bg: 'rgba(251,113,133,0.08)', label: '⚠ HIGH' },
  EXTREME: { color: 'rgba(239,68,68,1)',      bg: 'rgba(239,68,68,0.12)',   label: '⚡ EXTREME' },
} as const);

const PREDICTION_CONFIG = Object.freeze({
  PUMP:     { color: 'var(--zm-positive)',  bg: 'var(--zm-positive-bg)',  label: '▲ PUMP', icon: '↑' },
  DUMP:     { color: 'var(--zm-negative)', bg: 'rgba(251,113,133,0.10)', label: '▼ DUMP', icon: '↓' },
  SIDEWAYS: { color: 'rgba(0,238,255,0.70)', bg: 'rgba(0,238,255,0.06)', label: '◆ SIDEWAYS', icon: '→' },
} as const);
...
const MetricBar = memo(({ label, value, color, unit = '' }: MetricBarProps) => (
  <div>
    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
      <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '9px', letterSpacing: '0.1em', color: 'rgba(138,138,158,0.50)' }}>
        {label}
      </span>
    ...
    <div style={{ height: '3px', background: 'var(--zm-surface-2)', borderRadius: '2px', overflow: 'hidden' }}>
      <motion.div
...
const SignalGauge = memo(({ value }: SignalGaugeProps) => {
  ...
  const color = value > 0.2 ? 'var(--zm-positive)' : value < -0.2 ? 'var(--zm-negative)' : 'rgba(0,238,255,0.70)';
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '9px', color: 'rgba(138,138,158,0.50)', letterSpacing: '0.1em' }}>TREND STRENGTH</span>
        <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '10px', color }}>
          {value > 0 ? '+' : ''}{(value * 100).toFixed(0)}
        </span>
      </div>
      <div style={{ height: '4px', background: 'var(--zm-surface-2)', borderRadius: '2px', position: 'relative' }}>
...
const AISignalTile = memo(({ symbol = 'BTC' }: AISignalTileProps) => {
  ...
  const backendBadgeStyle = useMemo(() => ({
    fontFamily: "'Space Mono', monospace",
    fontSize: '8px',
    color: result.tfReady ? 'rgba(52,211,153,0.6)' : 'rgba(138,138,158,0.30)',
  ...
  return (
    <GlassCard style={CARD_STYLE}>
      ...
      <div style={headerStyle}>
        <div style={titleStyle}>
          <svg width="10" height="10" viewBox="0 0 10 10" fill="none" aria-hidden="true">
            <polygon points="5,1 9,8 1,8" stroke="rgba(180,130,255,0.80)" strokeWidth="1" fill="none" />
            <circle cx="5" cy="5" r="1.5" fill="rgba(180,130,255,0.80)" />
          </svg>
          AI SIGNAL · {symbol}
        </div>
        <motion.div
      ...
      >
        <div>
          <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '9px', color: 'rgba(138,138,158,0.40)', letterSpacing: '0.1em', marginBottom: '2px' }}>
            PREDICTION
          </div>
      ...
      <div style={featureGridStyle}>
        <div style={featureBoxStyle}>
          <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '8px', color: 'rgba(138,138,158,0.40)', letterSpacing: '0.08em', marginBottom: '3px' }}>RSI</div>
          <div style={{
            fontFamily: "'Space Mono', monospace", fontSize: '13px', fontWeight: 700,
            color: result.features.rsi > 70 ? 'var(--zm-negative)' : result.features.rsi < 30 ? 'var(--zm-positive)' : 'rgba(0,238,255,0.90)',
          }}>
            {result.features.rsi.toFixed(1)}
          </div>
        </div>
        <div style={featureBoxStyle}>
          <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '8px', color: 'rgba(138,138,158,0.40)', letterSpacing: '0.08em', marginBottom: '3px' }}>MACD</div>
          <div style={{
            fontFamily: "'Space Mono', monospace", fontSize: '13px', fontWeight: 700,
            color: result.features.macdSignal > 0 ? 'var(--zm-positive)' : 'var(--zm-negative)',
          }}>
            {result.features.macdSignal > 0 ? '+' : ''}{(result.features.macdSignal * 1000).toFixed(2)}
          </div>
        </div>
        <div style={featureBoxStyle}>
          <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '8px', color: 'rgba(138,138,158,0.40)', letterSpacing: '0.08em', marginBottom: '3px' }}>BB POS</div>
          <div style={{
            fontFamily: "'Space Mono', monospace", fontSize: '13px', fontWeight: 700,
            color: result.features.bollingerPos > 0.8 ? 'var(--zm-negative)' : result.features.bollingerPos < 0.2 ? 'var(--zm-positive)' : 'rgba(0,238,255,0.90)',
          }}>
            {(result.features.bollingerPos * 100).toFixed(0)}%
          </div>
        </div>
        <div style={featureBoxStyle}>
          <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '8px', color: 'rgba(138,138,158,0.40)', letterSpacing: '0.08em', marginBottom: '3px' }}>VOL ANOM</div>
          <div style={{
            fontFamily: "'Space Mono', monospace", fontSize: '13px', fontWeight: 700,
            color: result.features.volumeAnomaly > 0.5 ? 'var(--zm-warning)' : 'rgba(138,138,158,0.70)',
          }}>
      ...
      <div style={backendBadgeStyle} aria-label={'AI backend: ' + (result.tfReady ? 'TensorFlow.js ' + result.backend : 'Pure JS fallback')}>
        <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: result.tfReady ? 'rgba(34,255,170,0.80)' : 'rgba(138,138,158,0.30)', display: 'inline-block' }} />
